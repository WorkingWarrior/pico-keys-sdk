...
void do_flash() {
    if (mutex_try_enter(&mtx_flash, NULL) == true) {
                ...
                if (flash_pages[r].ready == true) {
                    ...
                    ready_pages--;
                        ...
                        flash_range_erase(flash_pages[r].address - XIP_BASE, 
                            flash_pages[r].page_size ? 
                            ((int)(flash_pages[r].page_size / FLASH_SECTOR_SIZE)) * FLASH_SECTOR_SIZE : 
                            FLASH_SECTOR_SIZE);
                        while (multicore_lockout_end_timeout_us(1000) == false) {
                            ;
                        }
        ...
        flash_available = false;
            ...
            esp_partition_mmap(part0, 0, part0->size, ESP_PARTITION_MMAP_DATA, 
                             (const void **)&map, 
                             (esp_partition_mmap_handle_t *)&fd_map);
        #endif
    ...
    uint32_t data_end_addr;
        ...
        PICO_FLASH_SIZE_BYTES = part0->size;
            ...
            if (rc != 3) {
                ...
                uint16_t first_sector_number = (((uint32_t*)workarea)[1] & 
                    PICOBIN_PARTITION_LOCATION_FIRST_SECTOR_BITS) >> 
                    PICOBIN_PARTITION_LOCATION_FIRST_SECTOR_LSB;
                uint16_t last_sector_number = (((uint32_t*)workarea)[1] & 
...
page_flash_t *find_free_page(uintptr_t addr) {
    ...
    for (int r = 0; r < TOTAL_FLASH_PAGES; r++) {
        if ((!flash_pages[r].ready && !flash_pages[r].erase) ||
            flash_pages[r].address == addr_alg) {
            p = &flash_pages[r];
            if (!flash_pages[r].ready && !flash_pages[r].erase) {
                #ifdef PICO_PLATFORM
                ...
                #else
                    memcpy(p->page, 
                           (addr >= start_data_pool && addr <= end_rom_pool + sizeof(uintptr_t)) ? 
                           (uint8_t *)(map + addr_alg) : (uint8_t *)addr_alg, 
                           FLASH_SECTOR_SIZE);
                #endif
                ready_pages++;
                p->address = addr_alg;
...
int flash_program_block(uintptr_t addr, const uint8_t *data, size_t len) {
    ...
    page_flash_t *p = find_free_page(addr);
    if (!p) {
        mutex_exit(&mtx_flash);
...
bool flash_check_blank(const uint8_t *p_start, size_t size) {
    for (const uint8_t *p = p_start; p < p_start + size; p++) {
        if (*p != 0xff) {
            return false;
        }
    }
